...
int CBlueTooth::ScanNearbyBthDev (
		HANDLE hRadio,
		BOOL fReturnAuthenticated/* = TRUE*/,		//是否搜索已配对设备
		BOOL fReturnRemembered/* = TRUE*/,			//是否搜索已记忆的设备
		BOOL fReturnUnknown/* = TRUE*/,				//是否搜索未知设备
		BOOL fReturnConnected/* = TRUE*/,			//是否搜索已连接的设备
		BOOL fIssueInquiry/* = FALSE*/,				//是否重新搜索，TRUE 会执行新的搜索时间较长，FLASE 的时候会直接返回上次的搜索结果
		UCHAR cTimeoutMultiplier/* = 30*/			//指示查询超时值，以1.28为增量。例如，以12.8的查询cTimeoutMultiplier 为10，此成员的最大为48.当使用大于48的值调用函数失效立即返回
	)
{
	RemoveAll_RemoteBthDevInfo ();
