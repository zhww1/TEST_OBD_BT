...
int CMyEventView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	GetListCtrl().InsertColumn(0,_T("发生时间"),LVCFMT_LEFT,160);
	GetListCtrl().InsertColumn(1,_T("事件内容"),LVCFMT_LEFT,400);
...
LRESULT CMyEventView::OnResizeWnd(WPARAM wParam,LPARAM lParam)
{
	...
	if(rect.Width() > 0)
	{
		GetListCtrl().SetColumnWidth(0,280);
		GetListCtrl().SetColumnWidth(1,rect.Width() - 280);
	}
	return TRUE;
}
